...
int removeDirectory(const char *path_tar, const char *path_file_source){
  ...
  if(check_sys_call(fd_tar, "open in removeDirectory") == -1) return -2;

  int size_tar = lseek(fd_tar, 0, SEEK_END);
  if(check_sys_call(size_tar, "lseek in removeDirectory") == -1) return -1;

  int pos = lseek(fd_tar, 0, SEEK_SET);
  if(check_sys_call(pos, "lseek in removeDirectory") == -1) return -1;

  struct posix_header header;
  ssize_t size_read = read(fd_tar, &header, BLOCK_SIZE);
  if(check_sys_call(size_read, "read in removeDirectory") == -1) return -1;
  ...
  if(count != 0){
    ...
    if(check_sys_call(size_write, "write in removeDirectory") == -1) return -1;
